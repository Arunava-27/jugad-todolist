...
export default function Register({ onRegistered, onSwitchToLogin, onBack, inviteToken, inviteInfo }) {
  ...
  async function submit(e) {
    ...
    setLoading(true);
    try {
      const result = await api.register(email, name, password, workspaceName, inviteToken);
      onRegistered(result);
    } catch (err) {
    ...
    <div className="login-screen">
      <form className="login-card" onSubmit={submit}>
        ...
        <p className="login-sub">
          {inviteInfo ? `Create your account to join ${inviteInfo.workspaceName}` : 'Create your account'}
        </p>
        <label>
          Name
