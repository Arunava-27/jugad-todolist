...
router.get('/', (req, res) => {
  res.json(db.prepare('SELECT * FROM assignees WHERE workspace_id = ? ORDER BY sort_order').all(req.workspaceId));
});

router.post('/', (req, res) => {
  const { name } = req.body || {};
  if (!name || !name.trim()) return res.status(400).json({ error: 'name is required' });
  const maxOrder = db.prepare('SELECT MAX(sort_order) m FROM assignees WHERE workspace_id = ?').get(req.workspaceId).m ?? -1;
  try {
    const info = db.prepare('INSERT INTO assignees (workspace_id, name, sort_order) VALUES (?, ?, ?)')
      .run(req.workspaceId, name.trim(), maxOrder + 1);
