...
// belongs to it — admins may access any workspace. Must run after requireAuth.
export function requireWorkspace(req, res, next) {
  ...
  if (req.user.role !== 'admin') {
    const membership = db.prepare('SELECT * FROM workspace_members WHERE workspace_id = ? AND user_id = ?').get(workspaceId, req.user.id);
